...
def MIMO2MISO(channel,Nt,Nr,dk,K,B):
    ...
    H_bar = tf.reshape(H_bar,[-1,1,B*K*dk,B*K*dk])
    H_miso_bar = tf.concat([tf.math.real(H_bar),tf.math.imag(H_bar)],axis=1)
    #H_miso_bar = np.triu(np.real(H_bar)) + np.tril(np.imag(H_bar))
    H_miso_bar = tf.transpose(tf.reshape(H_miso_bar,[-1,2,B*K*dk,B*K*dk]),[0,2,3,1])
...
def data_process(data_root,Nt,Nr,dk,K,B,SNR_dB,SNR_channel_dB,test_length,data_mode):
    ...
    H_miso_bar = np.zeros((data_num,K*dk*B,K*dk*B,2))
