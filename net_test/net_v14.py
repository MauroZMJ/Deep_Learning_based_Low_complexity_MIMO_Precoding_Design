...
def backbone(data,Nt,Nr,dk,K,B):
    ...
    if net=='sigmoid':
        ...
        y = H_bar
        for i in range(3):
            y_temp = Conv2D(filters=2, kernel_size=(3,3),padding='same')(y)
            y_temp = BatchNormalization()(y_temp)
            y_temp = LeakyReLU()(y_temp)
            y_temp = Conv2D(filters=4, kernel_size=(3,3),padding='same')(y_temp)
            y_temp = BatchNormalization()(y_temp)
            y_temp = LeakyReLU()(y_temp)
            y_temp = Conv2D(filters=1, kernel_size=(3,3),padding='same')(y_temp)
            y_temp = BatchNormalization()(y_temp)
            y_temp = LeakyReLU()(y_temp)
            y = Add()([y,y_temp])

        # conv_arg_list = [[16,5],[32,3],[64,3],[64,3]]
        # for conv_arg in conv_arg_list:
        #     y = Conv2D(filters=conv_arg[0], kernel_size=(conv_arg[1],conv_arg[1]),padding='same')(y)
        #     y = BatchNormalization()(y)
        #     y = LeakyReLU()(y)            
        # y = Conv2D(filters=1, kernel_size=(3, 3),padding='same')(y)
        y = Add()([H_bar,y])             
        y = Flatten()(y)
        ...
        x_q = BatchNormalization()(y)
        q_list = Dense(K * dk * B * 8)(x_q)
        q_list = BatchNormalization()(q_list)
        q_list = LeakyReLU()(q_list)
        q_list = Dense(K * dk * B * 4)(x_q)
        q_list = BatchNormalization()(q_list)
        q_list = LeakyReLU()(q_list)
