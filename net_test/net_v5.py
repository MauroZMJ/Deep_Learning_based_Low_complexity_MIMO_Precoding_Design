...
from tensorflow.keras.layers import Activation, Dense, Conv1D, Conv2D, Flatten, Permute, Reshape, Input, \
    BatchNormalization, Concatenate, Add, Lambda, GlobalAveragePooling1D, Concatenate, GlobalAvgPool1D, Multiply,GlobalAvgPool2D
...
def backbone(data,Nt,Nr,dk,K,B):
    ...
    if net=='sigmoid':
        ...
        x = BatchNormalization()(x)
        x = LeakyReLU()(x)
        x = ResnetIdentityBlock(kernel_size=3, filters=[4,16,8])(x)


        x = Conv2D(filters=4, kernel_size=(3, 3))(x)
        x = BatchNormalization()(x)
        x = LeakyReLU()(x)
        x = ResnetIdentityBlock(kernel_size=3, filters=[4,8,4])(x)    
...
fh = logging.FileHandler(dataset_root + 'result/tensorflow_%s_%d_%d_%d_%d_%d_%d_%d_%s.log' % (prefix,Nt, Nr, K, B,dk, SNR_dB,SNR_channel_dB,net_name))
fh.setLevel(logging.DEBUG)
logger.addHandler(fh)
logger.addHandler(ch)
logger.info('%s performance'%(net_name))
