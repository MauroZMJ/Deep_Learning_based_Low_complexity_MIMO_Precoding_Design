...
from tensorflow.keras.layers import LeakyReLU,LSTM,GRU
...
from net_v15_preprocessing import data_process
net_name = 'init_net'
...
def backbone(data,Nt,Nr,dk,K,B):
    ...
    net = 'sigmoid'
    if net=='sigmoid':
        data = Reshape((K*dk*B,K*dk*B,2))(data)
        #data = Permute((1,3,2))(data)
        #import ipdb;ipdb.set_trace()
        #x = data
        # for i in range(4):
        #     x = GRU(K*dk*B*2, dropout=0,recurrent_dropout=0,return_sequences=True)(x)
        # x = GRU(B, dropout=0, recurrent_dropout=0, return_sequences=True)(x)
        # x = x[0]
        #x = GRU(K*dk*B*2, dropout=0,recurrent_dropout=0,return_sequences=False)(x)
        if K*dk>12:
        ...
        x = Reshape((2*62,-1))(x)
        x = GRU(K * dk * B, dropout=0, recurrent_dropout=0, return_sequences=True)(x)
        x = GRU(K * dk * B, dropout=0, recurrent_dropout=0, return_sequences=False)(x)
...
def su_net(Nt,Nr, K, B, dk, lr):
    data = Input(shape=(K*dk*dk*K*B*B*2))
