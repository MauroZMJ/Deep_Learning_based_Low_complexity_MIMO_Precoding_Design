...
def backbone(data,Nt,Nr,dk,K,B):
    ...
    if net=='sigmoid':
        ...
        H = Reshape((Nt,B*2*K*dk))(data[:,(K*dk*B)**2:])
        conv_arg_list = [[16,7],[8,5],[4,3],[4,3],[4,3],[2,3]]
        x = H_bar
        ...
        x = Flatten()(x)
        y = H
        conv1d_arg_list = [[K*dk*B,7],[K*dk*B,5],[K*dk*B,3],[K*dk,3]]
        for conv_arg in conv1d_arg_list:
            y = Conv1D(filters=conv_arg[0], kernel_size=conv_arg[1])(y)
            y = BatchNormalization()(y)
            y = LeakyReLU()(y)               
        #y = Conv1D(filters=5, kernel_size=3)(y)
        y = Flatten()(y)
        ...
        x_for_q = BatchNormalization()(x)
        x_for_q = Dense(K * dk * B * 2)(x_for_q)
        x_for_q = Activation('relu')(x_for_q)

        y_for_q = BatchNormalization()(y)
        y_for_q = Dense(K * dk * B * 2)(y_for_q)
        y_for_q = Activation('relu')(y_for_q)

        z = y
        #z = Concatenate(axis=1)([x_for_q,y_for_q])
        q_list = Dense(K * dk * B * 2)(z)
...
def weighted_mse_loss(y_true, y_pred):
    ...
    loss = mse_pa + mse_lambda + 2*mse_q
    return loss
...
def DUU_EZF_loss(y_true, y_pred):
    ...
    channel = y_true[:, 2 * dk * Nt * K * B:-(K*dk + K*dk*B*3)]
    labelset_su = y_true[:,-(K*dk + K*dk*B*3):]

    pa_list_true = labelset_su[:, :K * dk]
    lambda_list_true =labelset_su[:, K * dk: (K*dk + K*dk*B)]
    q_list_true = labelset_su[:,(K*dk + K*dk*B):]
    ...
    V_restore = pq2V(channel=channel_miso,Nt=Nt,Nr=1,dk = 1,K=K*dk,B=B,p=p,sigma_2=sigma_2,lambda_list=lambda_list_true,pa_list=pa_list_true,q_list=q_list_pred)
...
labelset_un = np.concatenate([dataset , H_noiseless, labelset_su], axis=-1)
if train_mode == 'train':
