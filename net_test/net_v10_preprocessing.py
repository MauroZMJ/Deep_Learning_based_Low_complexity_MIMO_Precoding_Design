...
def MIMO2MISO(channel,Nt,Nr,dk,K,B):
    ...
    H_ensemble_p = tf.reshape(tf.transpose(P,(0,3,4,1,2)),(-1,K*dk*1,Nt))
    H_bar_p = tf.matmul(H_ensemble_p, H_ensemble_p, adjoint_b=True)
    H_miso_bar_p = np.triu(np.real(H_bar_p)) + np.tril(np.imag(H_bar_p))
    H_miso_bar_p = tf.transpose(tf.reshape(H_miso_bar_p,[-1,B,K*dk,K*dk]),[0,2,3,1])
    
    P = tf.cast(tf.concat([tf.math.real(P), tf.math.imag(P)], axis=4), dtype=tf.float32)
    return P,H_miso_bar,H_miso_bar_p
def data_process(data_root,Nt,Nr,dk,K,B,SNR_dB,SNR_channel_dB,test_length,data_mode):
    ...
    H_miso_bar_p = np.zeros((data_num,K*dk,K*dk,B))
    total_iter = len(H_miso)//1000
    for i in range(total_iter):
        H_iter = H[i * 1000:(i + 1) * 1000, :]
        H_iter = np.concatenate([np.real(H_iter), np.imag(H_iter)], axis=-1)
        H_miso_iter,H_miso_bar_iter,H_miso_bar_p_iter = MIMO2MISO(channel=H_iter, Nt=Nt, Nr=Nr, dk=dk, K=K, B=B)
        H_miso[i * 1000:(i + 1) * 1000, :] = H_miso_iter.numpy()
        H_miso_bar[i * 1000:(i + 1) * 1000, :] = H_miso_bar_iter
        H_miso_bar_p[i * 1000:(i + 1) * 1000, :] = H_miso_bar_p_iter
    H_miso = tf.reshape(H_miso,(data_num,-1))
    #import ipdb;ipdb.set_trace()
    H_miso_bar = np.concatenate([np.reshape(H_miso_bar,(data_num,-1)),np.reshape(H_miso_bar_p,(data_num,-1))],axis=1)
