...
origin_SNR_channel_dB = SNR_channel_dB
data_mode = args.mode
...
su_model_path = dataset_root + 'model/DUU_MISO_models_%d_%d_%d_%d_%d_%d_su.h5' % (Nt, Nr, K, dk, SNR_dB,origin_SNR_channel_dB)
...
if train_mode == 'train':
    model.fit(dataset_bar, labelset_su, epochs=epochs, batch_size=batch_size, verbose=0, \
...
un_model_path = dataset_root + 'model/DUU_MISO_models_%d_%d_%d_%d_%d_%d_un.h5' % (Nt, Nr, K, dk, SNR_dB,origin_SNR_channel_dB)
...
if train_mode == 'train':
    model.fit(dataset_bar, labelset_un, epochs=epochs, batch_size=batch_size, verbose=0, \
              validation_split=0.1, callbacks=[checkpointer, reduce_lr, early_stopping])
...
_, _, WMMSE_output, _ = WMMSE(channel)
...
fh = logging.FileHandler(dataset_root + 'result/Imperfect_CSI_%s_%d_%d_%d_%d_%d_%d.log' % (prefix,Nt, Nr, K, dk, SNR_dB,SNR_channel_dB))
