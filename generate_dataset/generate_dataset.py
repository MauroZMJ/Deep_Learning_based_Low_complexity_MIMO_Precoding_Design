...
if Nt==64:
    ...
    data_root = dataset_root + 'channel_dataset_small.mat'
if data_mode =='debug':
...
for i in range(total_iter):
    ...
    #np.mean(minus_sum_rate_loss(channel_iter_miso, V_iter_miso,Nt,1,1,K*dk,p,sigma_2))
    #V_iter_miso_test = tf.reshape(V_iter_miso,(-1,Nt,dk,K,2))
    #np.mean(minus_sum_rate_loss(channel_iter, V_iter_miso_test,Nt,Nr,dk,K,p,sigma_2))
    #sinr_iter = calculate_sinr(channel_iter_miso,Nt = Nt,Nr = 1,dk = 1, K = K*dk, p=p,sigma_2=sigma_2,V=V_iter_miso)
    ...
    transmit_pa[i * 1000:(i + 1) * 1000, :] = pa_list.numpy()
    upload_pa[i * 1000:(i + 1) * 1000, :] = lambda_list.numpy()
    rb_allocate_vec[i * 1000:(i + 1) * 1000, :] = q_list.numpy()

data_save_root = dataset_root + 'data/DUU_MISO_dataset_%d_%d_%d_%d_%d_%d.mat' % (Nt, Nr, K, dk,B, SNR_dB)
...
fh = logging.FileHandler(dataset_root + 'data/DUU_MISO_dataset_%d_%d_%d_%d_%d_%d.log' % (Nt, Nr, K, dk,B, SNR_dB))
