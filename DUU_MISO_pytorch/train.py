...
def test(data_loader,model,criterion):
    ...
    model.train()
    return np.mean(loss_list)
def train(data_loader, valid_data_loader, model, criterion,save_path,lr,mode='su'):
    min_loss = 1e5
    if mode=='su':
        threshold = 1e-6
    else:
    ...
    for epoch in range(epochs):
        ...
        if valid_loss < min_loss and abs(valid_loss - min_loss) > 1e-4:
            min_loss = valid_loss
...
if __name__ == "__main__":
    ...
    train(train_su_dataloader,valid_su_dataloader, model, criterion=MSE_loss,save_path = best_su_model_path,  lr = 1e-2,mode='su')
    ...
    train(train_un_dataloader,valid_un_dataloader, model, criterion=SMR_loss,save_path = best_un_model_path, lr = 1e-3,mode='un')
    ...
    fh = logging.FileHandler(base_root + 'model/DUU_pytorch/DUU_models_%d_%d_%d_%d_%d.log'%(Nt,Nr,K,dk,SNR_dB))
