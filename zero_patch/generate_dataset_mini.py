...
def WMMSE(channel,Nt,Nr,dk,K,B,p,sigma_2,UW_return):
    ...
    if UW_return:
        ...
        return V
def generate_dataset_custom_func(channels,Nt,Nr,dk,K,B,p,sigma_2):
    #channels shape:data_num*Nt*Nr*B* K
    ...
    channels = channels[:,:,:,:,:K] + 1j*channels[:,:,:,:,K:]
    ...
    total_dataset = {'transmit_power_allocation': transmit_pa, 'upload_power_allocation': upload_pa, 'resource_block_allocation':rb_allocate_vec,
                'H': channels}
