...
def backbone(data,Nt,Nr,dk,K,B):
    ...
    def vector_norm_l2(vec):
        ...
        return v_norm
    def swish(vec):
        return Multiply()([vec,tf.sigmoid(vec)])
    net = 'origin'
    if net == 'origin':
        ...
        data = Reshape((K * dk * B, K * dk * B, 1))(data)
        conv_config_list = [[16, 7], [8, 5], [2, 3]]
        x1 = data
        for conv_config in conv_config_list:
            x1 = Conv2D(filters=16, kernel_size=(7, 7))(x1)
            x1 = BatchNormalization()(x1)
            x1 = Activation('elu')(x1)
        x2 = data
        x1 = Flatten()(x1)
        x_tp = BatchNormalization()(x1)
        tp_list = Dense(K * dk)(x_tp)
        tp_list = BatchNormalization()(tp_list)
        tp_list = Activation('elu')(tp_list)
        tp_list = Dense(K * dk)(tp_list)
        tp_list = Activation('sigmoid')(tp_list)
        tp_list = Lambda(vector_norm_l1)(tp_list)
        x_up = BatchNormalization()(x1)
        up_list = Dense(K * dk * B)(x_up)
        up_list = BatchNormalization()(up_list)
        up_list = Activation('elu')(up_list)
        up_list = Dense(K * dk * B)(up_list)
        up_list = Activation('sigmoid')(up_list)
        up_list = Lambda(vector_norm_l1)(up_list)
        # x_q = BatchNormalization()(x2)
        q_list_list = []
        for m in range(K * dk):
            x_q = x2[:, m:(B * K * dk):(K * dk), m:(B * K * dk):(K * dk), :]
            x_q = Flatten()(x_q)
            q_list = x_q
            for j in range(2):
                q_list = Dense(50)(q_list)
                q_list = BatchNormalization()(q_list)
                q_list = Activation('elu')(q_list)
            q_list = Dense(B * 2)(q_list)
            q_list = Activation('tanh')(q_list)
            q_list = Lambda(vector_norm_l2)(q_list)
            q_list_list.append(q_list)
        q_final_list = Concatenate(axis=1)([q_list_list[i] for i in range(K * dk)])
        prediction = Concatenate(axis=1)([tp_list, up_list, q_final_list])
    return prediction
