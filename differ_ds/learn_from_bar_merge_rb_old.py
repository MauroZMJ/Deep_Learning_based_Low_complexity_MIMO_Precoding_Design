...
su_model_path = dataset_root + 'model/DUU_MISO_models_differ_ds_%d_%d_%d_%d_%d_%d_su.h5' % (Nt, Nr, K, dk, SNR_dB,SNR_channel_dB)
...
def pq2V_low_complexity(channel,Nt,Nr,dk,K,p,sigma_2,q_list,p_list):
    ...
    q_list = tf.cast(q_list*0+1, tf.complex128)
    V_norm = list()
...
un_model_path = dataset_root + 'model/DUU_MISO_models_differ_ds_%d_%d_%d_%d_%d_%d_un.h5' % (Nt, Nr, K, dk, SNR_dB,SNR_channel_dB)
...
for it in range(iter_num):
    DUU_EZF_output = restore(test_dataset[it*batch_size:(it+1)*batch_size,:], model_output[it*batch_size:(it+1)*batch_size,:],test_ds_dataset[it*batch_size:(it+1)*batch_size,:])
    DUU_EZF_performance[it] = np.mean(minus_sum_rate_loss(channel_noiseless[it*batch_size:(it+1)*batch_size,:], DUU_EZF_output,Nt=Nt,Nr=Nr,dk=dk,K=K,B=B,p=p,sigma_2=sigma_2))

    DUU_MISO_output = restore(test_dataset[it*batch_size:(it+1)*batch_size,:], test_labelset_su[it*batch_size:(it+1)*batch_size,:],test_ds_dataset[it*batch_size:(it+1)*batch_size,:])
    DUU_MISO_performance[it] = np.mean(minus_sum_rate_loss(channel_noiseless[it*batch_size:(it+1)*batch_size,:], DUU_MISO_output,Nt=Nt,Nr=Nr,dk=dk,K=K,B=B,p=p,sigma_2=sigma_2))
    #import ipdb;ipdb.set_trace()

    EZF_output = EZF(channel[it*batch_size:(it+1)*batch_size,:],Nt,Nr,dk,K,B,p,sigma_2)
    EZF_performance[it] = np.mean(minus_sum_rate_loss(channel_noiseless[it*batch_size:(it+1)*batch_size,:], EZF_output,Nt=Nt,Nr=Nr,dk=dk,K=K,B=B,p=p,sigma_2=sigma_2))
    WMMSE_output= WMMSE(channel[it*batch_size:(it+1)*batch_size,:],Nt,Nr,dk,K,B,p,sigma_2)
    RWMMSE_performance[it] = np.mean(minus_sum_rate_loss(channel_noiseless[it*batch_size:(it+1)*batch_size,:], WMMSE_output,Nt=Nt,Nr=Nr,dk=dk,K=K,B=B,p=p,sigma_2=sigma_2))
