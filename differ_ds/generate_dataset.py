...
factor = 1
test_batch = 200
ds_dataset = np.ones((channels.shape[0],K))
for data_index in range(channels.shape[0]//test_batch):
    ...
    ds_dataset[data_index*200 : (data_index+1)*200,:] = np.repeat(ds_this_list,test_batch,axis = 1).T
...
for i in range(total_iter):
    ...
    ds_iter = ds_dataset[i * batch_size:(i + 1) * batch_size, :]
