...
import hdf5storage
np.random.seed(2021)
def H2H_bar(H,Nt,Nr,dk,K,data_num):
    H = np.reshape(H,(-1,Nt,Nr,2*K))
    H = H[:,:,:,:K] + 1j*H[:,:,:,K:]
    ...
    print(H_bar.shape)
def MIMO2MISO(channel,Nt,Nr,dk,K):
    ...
    return P,H_miso_bar
def data_process(data_root,Nt,Nr,dk,K,SNR_dB,SNR_channel_dB,test_length,data_mode):
    total_dataset = hdf5storage.loadmat(data_root)
    if data_mode == 'debug':
        data_length = 4000
        # total_dataset={}
        # total_dataset['H'] = np.random.randn(data_length,Nt,Nr,1,K)
        # total_dataset['transmit_power_allocation'] = np.ones((test_length,K*dk))/(K*dk)
        # total_dataset['upload_power_allocation'] = np.ones((test_length,K*dk))/(K*dk)
    else:
        data_length = len(total_dataset['H'])
    H = total_dataset['H'][:data_length, :, :, 0, :]
    ...
    labelset_su = np.concatenate((transmit_pa, upload_pa), axis=-1)
    ...
    H_miso = np.zeros((data_num,Nt,1,2*K*dk))
    H_miso_bar = np.zeros((data_num,K*dk,K*dk))
    total_iter = len(H_miso)//1000
    for i in range(total_iter):
        H_iter = H[i * 1000:(i + 1) * 1000, :]
        H_miso_iter,H_miso_bar_iter = MIMO2MISO(channel=H_iter, Nt=Nt, Nr=Nr, dk=dk, K=K)
    ...
    return dataset,test_dataset,dataset_bar,test_dataset_bar,H,test_H,H_noiseless,test_H_noiseless,labelset_su,test_labelset_su