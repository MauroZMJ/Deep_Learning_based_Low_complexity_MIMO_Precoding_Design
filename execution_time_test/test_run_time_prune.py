...
##################
CNN_config_list = [16-args.CNN1_prune,8-args.CNN2_prune,4-args.CNN3_prune]
# %% supervised training
def backbone(data,Nt,Nr,dk,K):
    ...
    if net=='softmax':
        ...
        if K*dk>12:
            x = Conv2D(filters=CNN_config_list[0], kernel_size=(7, 7))(data)
            x = BatchNormalization()(x)
            x = LeakyReLU()(x)
        else:
            x = data
        try:
            x = Conv2D(filters=CNN_config_list[1], kernel_size=(5, 5))(x)
        except:
            x = Conv2D(filters=CNN_config_list[1], kernel_size=(5, 5),padding='same')(x)
        x = BatchNormalization()(x)
        x = LeakyReLU()(x)
        try:
            x = Conv2D(filters=CNN_config_list[2], kernel_size=(3, 3))(x)
        except:
            x = Conv2D(filters=CNN_config_list[2], kernel_size=(3, 3),padding='same')(x)    
        x = BatchNormalization()(x)
        x = LeakyReLU()(x)
        x = Flatten()(x)
        tp_list = Dense(K * dk)(x)
...
model_output = serve(test_dataset_bar[0:1,:])
...
for i in range(test_num):
    i_round = i % len(test_H)
    model_output = serve(test_dataset_bar[i_round:i_round+1,:])
...
t = time.time()
for i in range(test_num):
    i_round = i % len(channel)
    EZF_output = EZF(channel[i_round:i_round+1,:])
...
logger.info('%s_Nt_%d_Nr_%d_K_%d_dk_%d_SNR_%d_SNR_channel_%d_prune_CNN_1_%d_CNN_2_%d'%(prefix,Nt, Nr, K, dk, SNR_dB,SNR_channel_dB,args.CNN1_prune,args.CNN2_prune))
